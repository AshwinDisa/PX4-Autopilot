...
class controller():
    ...
    def waypoint_publisher(self, waypoints):
        ...
        # displacement for xyz
        # displacement = math.sqrt(pow((x - self.current_x),2) +
        #                     pow((y - self.current_y),2)
        #                     + pow((z - self.current_z),2))

        # displacement for gps
        displacement = math.sqrt(pow((x - self.current_lat),2) +
                            pow((y - self.current_long),2)
                            + pow((z - self.current_alt),2))
...
if __name__ == "__main__":
    ...
    try:
        ...
        waypoints = np.mat(np.array([[17.446237, 78.346342], [17.446339, 78.346451], [17.446161, 78.346448]]))
